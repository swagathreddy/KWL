...
// --- FIX 1: Import the necessary types from the library ---
import {
  DragDropContext,
  Droppable,
  Draggable,
...
function App() {
  ...
  // --- FIX 2: Add the 'DropResult' type to the result parameter ---
          ...
          <div ref={boardRef} className="bg-white rounded-lg shadow-lg p-4 lg:p-6 relative">
            ...
            <DragDropContext onDragEnd={onDragEnd}>
              <Droppable droppableId="notes-grid">
                {/* --- FIX 3: Add the 'DroppableProvided' type --- */}
                  ...
                  <div {...provided.droppableProps} ref={provided.innerRef} className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-3 min-h-[450px] p-4 bg-gray-50 rounded-lg">
                    {getVisibleNotes().map((note, index) => (
                      <Draggable key={note.id} draggableId={note.id} index={index}>
                        {/* --- FIX 4: Add the 'DraggableProvided' and 'DraggableStateSnapshot' types --- */}
                        {(provided: DraggableProvided, snapshot: DraggableStateSnapshot) => (
                          <div ref={provided.innerRef} {...provided.draggableProps} {...provided.dragHandleProps} className={`
                              ${colorClasses[note.color]} h-32 p-2 rounded-lg border-2 shadow-md cursor-move transform transition-all duration-200
                              ${snapshot.isDragging ? 'rotate-3 scale-105 shadow-xl z-10' : 'hover:shadow-lg'} relative group flex flex-col`}>
